      ...
      >
          ...
          <div className="glass-card p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                <button onClick={() => navigate('/level-guide')} className="text-xs text-gold-700 font-semibold hover:underline cursor-pointer">Level {progress.data?.level || currentAgent?.level || 1}</button>
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('/hall-of-fame')} className="text-xs text-gold-700 flex items-center gap-1">
            ...
            {(['Bulanan', 'All-Time'] as const).map(p => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                  hofPeriod === p ? 'bg-gold-200 text-dark-bg' : 'bg-white/10 text-zinc-400'
                }`}
              >
          ...
          {hof.loading ? (
            ...
            <Card className="flex items-center gap-3 p-4">
              ...
              <button onClick={fetchAll} className="text-xs text-gold-700 shrink-0">{t('common.reload') || 'Muat ulang'}</button>
            </Card>
          ) : !hof.data?.length ? (
                      ...
                      <div className="relative">
                        ...
                        {top3.map((entry, ei) => (
                          <div key={entry.id} className={`flex items-center justify-between py-1.5 ${ei > 0 ? 'border-t border-white/5' : ''}`}>
                            <div className="flex items-center gap-2">
                              <span className={`w-5 text-center text-xs font-bold ${ei === 0 ? 'text-gold-700' : ei === 1 ? 'text-zinc-300' : 'text-orange-400'}`}>
                                #{entry.rank}
                              </span>
                              <div>
                            ...
                            {ei === 0 && <Crown className="w-4 h-4 text-gold-700" />}
                          </div>
                        ))}
            ...
            <Card>
              ...
              <div className="flex gap-2 mb-3">
                {(['xp', 'commission'] as const).map(type => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                      lbType === type ? 'bg-gold-200 text-dark-bg' : 'bg-white/10 text-zinc-400'
                    }`}
                  >
                ...
                {[1, 0, 2].map(pos => {
                  ...
                  return (
                    <div key={entry.rank} className={`flex flex-col items-center gap-1 ${heights[pos]} justify-end`}>
                      {isFirst && <Crown className="w-5 h-5 text-gold-700" />}
                      <div className={`rounded-xl ${isFirst ? 'gold-gradient' : 'bg-white/10'} ${avatarSizes[pos]} flex items-center justify-center shadow-[0_0_20px_rgba(229,169,60,0.3)]`}>
                        <span className={`text-lg font-bold ${isFirst ? 'text-dark-bg' : 'text-zinc-400'}`}>#{entry.rank}</span>
                      </div>
                      ...
                      <p className="text-[10px] text-gold-700 font-semibold">{formatCompact(entry.total_commission)}</p>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-1">
                {lbFiltered.slice(3, 10).map(entry => (
                  <div
                    key={entry.rank}
                    className={`flex items-center justify-between p-2 rounded-xl ${entry.agent.id === currentAgent?.id ? 'bg-gold-200/10 border border-gold-700/20' : ''}`}
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-6 text-center text-sm font-bold text-zinc-500">#{entry.rank}</span>
                      <span className={`text-sm ${entry.agent.id === currentAgent?.id ? 'text-gold-700' : 'text-white'}`}>
                        {entry.agent.name}
                      </span>
                    </div>
          ...
          {activities.loading ? (
            ...
            <Card>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => navigate('/activities')} className="text-xs text-gold-700 flex items-center gap-1">
                  {t('home.view_all')} <ChevronRight className="w-3 h-3" />
                </button>
              </div>
